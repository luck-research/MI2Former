...
class FocusedLinearAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, focusing_factor=3, kernel_size=5):
        ...
        print('Linear Attention f{} kernel{} dim{} num_heads{}'.format(focusing_factor, kernel_size, dim, num_heads))
...
class MIFABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True,
                 ...
                 fcla_focusing_factor=3, fcla_kernel_size=5, atten_type="L",
                 **kwargs):
        super().__init__() 
        self.dim = dim
        self.num_heads = num_heads
        ...
        if atten_type == "L":
            ...
            self.attn = FocusedLinearAttention(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, focusing_factor=fcla_focusing_factor, kernel_size=fcla_kernel_size)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
        ...
        self.blocks = nn.ModuleList([
            MIFABlock(dim=dim, num_heads=num_heads,
                      mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
                      drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer=norm_layer,
                      fcla_focusing_factor=fcla_focusing_factor, fcla_kernel_size=fcla_kernel_size, atten_type=atten_type
                      )
...
class MIFAABackbone(nn.Module):
    ...
    Args:
        ...
        depths (tuple(int)): Depth of each MIFA layer.
